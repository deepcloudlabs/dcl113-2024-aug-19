...
int main() {
    ...
    auto it_emps = employees | ranges::views::filter( [](const employee& emp){return emp.getMDepartment() == employee::it;});
    for (auto& emp : it_emps){
        cout << emp << endl;
    }
    auto female_emps = employees | ranges::views::filter( [](const employee& emp){return emp.getMGender() == employee::female;});
    for (auto& emp : female_emps){
        cout << emp << endl;
    }
    return 0;
}
